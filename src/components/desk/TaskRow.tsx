...
export function TaskRow({
  ...
  actions?: ReactNode;
  className?: string;
  /** The words the top bar's search matches this row against. Absent leaves the row alone. */
  search?: string;
}) {
  return (
    <div data-search={search} className={`flex flex-wrap items-center gap-x-3.5 gap-y-2.5 border-t border-line py-3 ${className}`}>
