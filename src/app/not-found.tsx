...
export const metadata: Metadata = {
  ...
  description: "The page may have moved, changed, or been taken down.",
  robots: { index: false, follow: false },
};

// A printed program with one entry struck out. A program belongs to a concert, a match, a screening
// and a play alike, so the joke is nobody's in particular. Decorative: the heading carries the message.
const SET = ["", "", "This page", "", ""];
      ...
      <main id="main" className="relative flex-1 overflow-hidden">
        ...
        <div className="relative mx-auto w-full max-w-[1120px] px-7 pb-[90px] pt-[88px]">
          <div className="grid items-start gap-12 md:grid-cols-[minmax(0,1fr)_auto] md:gap-16">
            <div>
              <Eyebrow className="mb-8">404: Page not found</Eyebrow>
              <h1 className="display max-w-[12ch] text-[clamp(44px,7.4vw,96px)] leading-[0.98]">
                This page is <em className="text-accent-ink">not here.</em>
              </h1>
            ...
            <div aria-hidden="true" className="relative mt-2 w-full max-w-[300px] md:mt-3 md:w-[280px]">
              <div className="glow bg-ground px-6 pb-7 pt-5">
                <div className="heading text-[24px] leading-tight">Program</div>
                <div className="caps mt-1 text-[14px] text-muted">One entry removed</div>
                <ol className="mt-4 text-[15px] leading-none">
                  {SET.map((entry, i) => (
                    <li key={i} className="flex min-h-[42px] items-baseline gap-3 border-b border-line py-3">
                      ...
                      {entry && <span className="caps text-[14px] text-muted line-through decoration-accent decoration-2">{entry}</span>}
                    </li>
                  ))}
                </ol>
              </div>
              <Stamp className="absolute -bottom-10 -right-8 bg-ground">
                NOT<br />ON THE<br />PROGRAM
              </Stamp>
            </div>
          </div>
        </div>
