...
const MARK = {
  ...
  red: "rounded-full bg-red text-white",
...
function Mark({ text, kind }: { text: string; kind: keyof typeof MARK }) {
  return (
    <span aria-hidden="true" className={`poster flex h-[26px] w-[26px] flex-none items-center justify-center border-2 border-ink text-[11px] leading-none tracking-[0.02em] ${MARK[kind]}`}>
      {text}
    </span>
  );
}
    ...
    <>
      <div className="hard-border mb-2.5 mt-9 flex flex-wrap items-center justify-between gap-5 bg-ink px-7 py-[26px] text-paper shadow-[8px_8px_0_var(--red)]">
        <div>
          ...
          <span className="typewriter text-[13px] text-[#9B968A]">what the board is worth right now</span>
        </div>
        <div className="text-right">
          ...
          <span className="typewriter text-[12.5px] text-[#9B968A]">{closesLabel}</span>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div>
                ...
                <div className="typewriter mt-1 text-[12.5px] text-gray">{l.note}</div>
              </div>
              <div className="min-w-[190px] min-[681px]:text-right">
                <div className="typewriter text-[13px] text-gray">{label}</div>
                ...
                <div className="flex items-center gap-[7px] min-[681px]:justify-end">
                  ...
                  <div className="typewriter text-[12.5px] text-gray">{bidder}</div>
                </div>
                {!sold && (
                  <button
                    type="button"
                    onClick={() => press(l)}
                    className="poster mt-2 cursor-pointer border-[3px] border-ink bg-red px-[18px] py-[9px] text-[15px] tracking-[0.03em] text-white shadow-[4px_4px_0_var(--black)] transition-[transform,box-shadow] duration-[80ms] hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-[2px_2px_0_var(--black)]"
                  >
                    {button}
                  </button>
                )}
              ...
              {sold && (
                <div className="typewriter absolute -top-3.5 right-4 -rotate-[9deg] border-[3px] border-red bg-cream px-3 py-[3px] text-[15px] text-red shadow-[2px_2px_0_rgba(0,0,0,0.2)]">
                  SOLD
                </div>
              )}
            </div>
