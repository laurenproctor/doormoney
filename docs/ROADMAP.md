...
- [x] Release rule: what counts as "the run happening." See `docs/DECISIONS.md`, decision 2. The default in this codebase is calendar-based: funds release in equal weekly slices across the run's dates, with an act-side "cancel run" that refunds the remainder. Amended by the remediation plan's Phase 2: the calendar releases a fan backing on its own, but a sponsorship's slices also wait for the musician to approve the logo, because `/terms` promises a declined logo a full refund and a refund can only return what has not been sent. `slicePlan` in `src/lib/release.ts`, migration 0031 under it.
- [x] Refunds: the act can cancel an open or live run from the dashboard; every patron gets back the unreleased slices, fee included (`src/lib/refunds.ts`). A declined mark refunds in full and puts the spot back on the board. Refunds made in the Stripe Dashboard are mirrored by the `charge.refunded` webhook. Since the remediation plan's Phase 2 a refund is written down before Stripe is called (migration 0032) and retried by the daily job, so one that fails is still owed rather than forgotten. `src/lib/outbox.ts`.
...
- [x] Close job: the worker at `/api/cron/auctions` (`src/lib/auctions.ts`), called every five minutes from the database (migration 0036, pg_cron) and once a day from `/api/cron/daily` as a backstop. Pages never settle anything. Each close and roll is one transaction in Postgres (migration 0035). The top bid at or above the reserve wins and gets 48 hours; nothing at the reserve goes unsold and the act is told.
