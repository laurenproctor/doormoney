...
**Amended (2026-09-23, migration 0058): the one word is the person's, and an organization gets its own.** One word doing both jobs is right while the organizer is the account holder: whoever signs in as `gutter-hymns` is the one whose page is at `/gutter-hymns`. It stops being right the moment an account stands up a business, a team or a group. Harbor House is not a credential, and suggesting `/harbor-house` on the setup screen must never rename the word somebody signs in with. So there are two doors into the namespace rather than one. `claim_username` stays the person's word and still moves an organizer's address with it, but only the organizer that was holding that same word, which is every organizer made before 0058. `claim_act_slug` is the organizer's word: it claims an address for a new organizer, or renames one holding a word of its own, and never reads or writes `profiles.username`. Both check the same three places and take the same lock, so neither can hand out the other's word, and a retired word still redirects and is never given out again. Nothing stored changed: a first organizer may still take the account's own word, and that pair keeps moving together through `claim_username`.
