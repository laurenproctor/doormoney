...
test("a theater fundraiser saves a theater option at the organizer's price", async () => {
  ...
  assert.deepEqual(inserted(), [{ run_id: "run-1", surface_key: "foyer_banner", label: null, price_cents: 75000, mode: "fixed", status: "open", buy_now_cents: null }]);
...
test("a music fundraiser is still narrowed by its act type, and takes nothing from sports", async () => {
  ...
  assert.equal(result.ok, true);
  assert.deepEqual(inserted(), [
    { run_id: "run-1", surface_key: "case_lid", label: "Case lid spot 1", price_cents: 8000, mode: "auction", status: "open", buy_now_cents: 20000 },
    { run_id: "run-1", surface_key: "case_lid", label: "Case lid spot 2", price_cents: 8000, mode: "auction", status: "open", buy_now_cents: 20000 },
  ]);
...
test("a retired template starts nothing new, and a spot already on it stays editable", async () => {
  ...
  assert.deepEqual(updated?.payload, { label: null, price_cents: 80000, mode: "fixed", buy_now_cents: null });
