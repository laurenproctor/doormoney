...
test("the panel draws a form for an open deliverable and offers to publish one item at a time", () => {
  ...
  const html = renderToStaticMarkup(createElement(DeliveryPanel, { rows, youth: false }));
...
test("a youth team, and an item showing a minor, are offered no publish button at all", () => {
  ...
  const youth = text(renderToStaticMarkup(createElement(DeliveryPanel, { rows: shapeDelivery([raw({ evidence: item(false) })], []), youth: true })));
  assert.match(youth, /Youth team: stays private/);
  assert.doesNotMatch(youth, /Make public/);
  const minor = text(renderToStaticMarkup(createElement(DeliveryPanel, { rows: shapeDelivery([raw({ evidence: item(true) })], []), youth: false })));
...
test("a sponsorship that is no longer held shows its history and no form", () => {
  const closed = renderToStaticMarkup(createElement(DeliveryPanel, { rows: shapeDelivery([raw({ purchases: { id: "p1", lot_id: "lot-1", mark_status: "approved", payment_status: "refunded" } })], []), youth: false }));
